document.addEventListener('DOMContentLoaded', () => {
  ...
  Object.keys(sliderValueSpans).forEach(key => {
      const slider = advancedSettings[key.replace('Value', '')]; // 'guidance-scale-value' -> 'guidance-scale'
      if (slider) {
  ...
  async function handleGenerateImage() {
      ...
      const payload = {
          ...
          // 시드 값이 있으면 정수로 변환, 없으면 null 전송 (백엔드에서 랜덤 처리)
          seed: advancedSettings.seedInput.value ? parseInt(advancedSettings.seedInput.value, 10) : -1
      };

      try {
            // 백엔드 API 호출
            const response = await authService.fetchWithAuth('/generate-image', {
                method: 'POST',
                headers: {
            ...
            // 결과 이미지 표시
            displayResultImage(data.image_base64);
  ...
  /**
   * Base64 인코딩된 이미지 데이터를 화면에 표시하는 함수
   * @param {string} base64Image - Base64로 인코딩된 이미지 데이터
   */
  function displayResultImage(base64Image) {
